...
| packages/desktop-client/src/components/budget/goals/CategoryFundingContext.tsx      | Share live status between each category/month balance and detail.                                                                                                                |
| packages/desktop-client/src/components/budget/goals/CategoryFundingBalance.test.tsx | Cover badges without saved goals, refresh, removal, overspending, errors and feature-off behavior.                                                                               |
...
Previous implementation before the badge revision: `1a8d3e0e4ac449598837b667af3c4861c2e2ce0e`. [Latest Linux run](https://github.com/mattm4444/actual-budget-category-feature/actions/runs/35529409043): **passed**, verified from individual job steps and logs. Snapshot regeneration is disabled in this run. Workflow commit `76ba663a0` runs workspaces serially; application code is unchanged from `1a8d3e0e4`.
...
4. Use priority 1 with only 100 available toward a 250 shortfall: add all 250, show Funded and 150 Overbudgeted. Undo restores 400 budgeted and 100 available; redo restores 650 budgeted and 150 Overbudgeted. Repeat with zero/negative available funds. Ordinary Apply still clamps by priority.
